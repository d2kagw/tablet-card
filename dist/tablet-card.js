...
__decorate([
    property({ type: Boolean })
], ScreenSaverCard.prototype, "visible", void 0);
...
let TabletClockCard = class TabletClockCard extends LitElement {
    constructor() {
        super();
        this.date = new Date();
        setInterval(() => {
    ...
    setConfig(config) {
        ...
        this.config = Object.assign({ name: 'Tablet', frameless: false }, config);
    }
    shouldUpdate(changedProps) {
        return K(this, changedProps, true);
    }
...
__decorate([
    internalProperty()
], TabletClockCard.prototype, "date", void 0);
__decorate([
    internalProperty()
], TabletClockCard.prototype, "config", void 0);
TabletClockCard = __decorate([
    customElement('tablet-clock-card')
], TabletClockCard);

let TabletNoticeCard = class TabletNoticeCard extends LitElement {
    constructor() {
        super();
    }
    setConfig(config) {
        this.config = Object.assign({ name: 'Tablet' }, config);
    }
    shouldUpdate(changedProps) {
        if (this.hass.states &&
...
let TabletCard = class TabletCard extends LitElement {
    ...
    async _createUtilityStack() {
        this._utilityCards = await Promise.all(this._config.utility_cards.map(async (card) => this._createCard(card)));
    }
    async _createColumnCardStack(column) {
        return await Promise.all(column.cards.map(async (card) => this._createCard(card)));
    }
    async _createColumnCardStacks() {
        this._columnCards = await Promise.all(this._config.columns.map(async (column) => this._createColumnCardStack(column)));
    }
    render() {
          ...
          <div class="tablet-card-column tablet-card-column-0">
            <div class="tablet-card-column-0-starter">
              ...
              <div class="tablet-card-column-0-ender">
                ${this._utilityCards.map((card) => html `<div class="tablet-card-card">${card}</div>`)}
              </div>
          </div>
          ${this._columnCards.map((column, i) => {
            return html `
              <div class="tablet-card-column tablet-card-column-${i + 1}" >
                ${column.map(card => html `
                    <div class="tablet-card-card">
                      ${card}
                    </div>
                  `)}
...
__decorate([
    property()
], TabletCard.prototype, "_card", void 0);
__decorate([
    property()
], TabletCard.prototype, "_config", void 0);
__decorate([
    internalProperty()
], TabletCard.prototype, "_utilityCards", void 0);
__decorate([
    internalProperty()
], TabletCard.prototype, "_columnCards", void 0);
TabletCard = __decorate([
